...
Try {
	...
	[string]$appName = 'ITS Faculty Lab Shortcuts'
	[string]$appVersion = ''
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		New-Shortcut -Path "$envCommonDesktop\Support Lab Check-In and Check-Out.url" -TargetPath "http://msudenver.edu/etc/checkinform/" -ContinueOnError $true
		New-Shortcut -Path "$envCommonDesktop\Faculty and Staff Hub.url" -TargetPath "http://msudenver.edu/facstaff/" -ContinueOnError $true
		New-Shortcut -Path "$envCommonDesktop\Student Hub.url" -TargetPath "http://msudenver.edu/studenthub/" -ContinueOnError $true
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Remove-File -Path "$envCommonCommonDesktop\Support Lab Check-In and Check-Out.url" -ContinueOnError $true
		Remove-File -Path "$envCommonCommonDesktop\Faculty and Staff Hub.url" -ContinueOnError $true
		Remove-File -Path "$envCommonCommonDesktop\Student Hub.url" -ContinueOnError $true
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDRmJ0Y6DuY1bzG
# DVi+onhJEPU4wb9XT1JA3ikaL+iwCaCCD4cwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# gAAwGQYJKoZIhvcNAQkDMQwGCisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIIAr
# 6ct7UqAzIDmkR1FVl2yJ7NKxfZbOAEhID3b52zoLMA0GCSqGSIb3DQEBAQUABIIB
# AJ1TTKkZ90g4IWYQznlCAtEalgkhM9TcHtcy7BIQMjWZhfEGjA29aTusHMnK8MZO
# SLaJ0RZDZGQMmGBV7T6zYq2KinNyrQIGYpMLlnoWTQTycP8rvgL6cW+ZCRLUOdBJ
# NcJuu02s3qlF8tSMQcEGtWSyBtRT0DNPcsKaerarneHiJck3so8KtxCOpCTgRqlw
# g5Yp0Lr5sRhvNilsYB9k91dfbgcJnt7rKGtaNQ4pwN7gQXxhMHOZipQOfZ9A3kO3
# EwvoKPCRCIxhV9MEcjORTRo6QtP+fnVpAmSNmsk2TYXuDL18rXv9/TCJ20/RzBJY
# jwuTnSoGLDYqyqvyFmVVhLChggKiMIICngYJKoZIhvcNAQkGMYICjzCCAosCAQEw
...
# ATAcBgkqhkiG9w0BCQUxDxcNMTcwNDA2MTU1NTEwWjAjBgkqhkiG9w0BCQQxFgQU
# a77XU3JlctWircCgsT2xuBdsILkwgZ0GCyqGSIb3DQEJEAIMMYGNMIGKMIGHMIGE
...
# AQUABIIBAAXDE7bmEA63bvHxNIthpT2Ft1FKhNIjvU6AQh/blZIFCpAfN8h3EhRJ
# MPJn0w6ffCEmDS0L+NHTDY/Bv7j7LgHEndMGdXHWyRYk4r0KHL4xwGhe1oDn6/i/
# CEp/FmTBfzT+9R03pahJQcQb7O0JVFDrqTCPnqup2kKNWqa+S7rbBv6dD/fZHHB/
# nU3RKrZ7h29F0mxCXFrq8apqh/1f322dp6XgfKrwJvlwVK1y74aXqqWnzYs+yCnE
# ihNLAyKajifqFEJAFUwfHpW0YTUD7U32rR1IUDcfNceOhfOuN9FGVlWvAw2thBva
# N1saDwAY/LcKBox2Se+2BV4sjqYtuMk=
